...
const formSchema = z.object({
  nombre: z.string().min(2, {
    message: "El aeropuerto debe tener al menos 2 caracteres",
  }),
  latitud: z.number().min(-90).max(90),
  longitud: z.number().min(-180).max(180),
  associatedCity: z.string({
    required_error: "Por favor seleciona una ciudad asociada al aeropuerto",
  }),
});
...
export const CreateAirports = () => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    const request = await apiRequest(cityData, "/FlyEaseApi/Aeropuertos/Post", "post");
    
    if(!request.error){
      toast({
        description: "Se ha creado el aeropuerto exitosamente",      
      })
    }else{
      toast({
        description: "No se ha podido crear el aeropuerto, intente de nuevo.",
      })
    }
  };
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="Olaya herrera" {...field} />
                    </FormControl>
                    <FormDescription>El nombre del aeropuerto a ingresar.</FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
              />
