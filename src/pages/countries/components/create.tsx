...
export const CreateCountries = () => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    const request = await apiRequest(values, "/FlyEaseApi/Paises/Post", "post");
    if (!request.error) {
      toast({
        description: "Se ha creado el pais exitosamente",
      });
    } else {
      toast({
        description: "No se ha podido crear el pais, intente de nuevo.",
      });
    }
  };
