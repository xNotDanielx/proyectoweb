...
export const DeleteCostumers = () => {
  ...
  const handleDeleteClick = async () => {
    const request = await apiRequest(null, `/FlyEaseApi/Clientes/Delete/${selectedCostumer?.numerodocumento}`, "delete");
    if (!request.error) {
      toast({
        description: "Se ha borrado el cliente exitosamente",
      });
    } else {
      toast({
        description: "No se ha podido borrar el cliente, intente de nuevo.",
      });
    }
    mutate();
  };
            ...
            <AlertDialog>
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    Ten en cuenta que se Borrará el ultimo asiento del avión seleccionado!
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
